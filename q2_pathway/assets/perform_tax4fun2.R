...
database_mode <- argv[7]

cat(pwd_op_folder, query_otu_seq, query_otu_table, pwd_ref_data, iden, num_of_threads, database_mode)
...
# predict functions
runRefBlast(path_to_otus = query_otu_seq, path_to_reference_data = pwd_ref_data, path_to_temp_folder = ".", database_mode = database_mode, use_force = T, num_threads = num_of_threads)
makeFunctionalPrediction(path_to_otu_table = query_otu_table, path_to_reference_data = pwd_ref_data, path_to_temp_folder = ".", database_mode = database_mode, normalize_by_copy_number = norm_by_cn, min_identity_to_reference = iden, normalize_pathways = norm_path)
